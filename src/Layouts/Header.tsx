...
export default function Header() {
    ...
    const userData = {
        name: user?.user?.name? user?.user?.name : "Guest",
        email: user?.user?.email? user?.user?.email: "guest@localhost",
        imageUrl: `https://ui-avatars.com/api/?name=${user?.user?.name}`? `https://ui-avatars.com/api/?name=${user?.user?.name}` :
    ...
    return (
            <Disclosure as="nav" className="bg-gray-800 w-full h-screen">
                {({ open }) => (
                    <>
                            ...
                            <div className="flex items-center justify-between h-16">
                                ...
                                <div className="-mr-2 flex md:hidden">
                                    ...
                                    <Disclosure.Button className="bg-gray-800 inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                                        ...
                                        {open ? <Close className="block h-6 w-6" /> : <Nav className="block h-6 w-6" />}
                                    </Disclosure.Button>
                                </div>
                            </div>
                        </div>
